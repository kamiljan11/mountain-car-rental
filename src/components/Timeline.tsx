...
import CustomerFormModal, { type CustomerDocValues } from "@/components/CustomerFormModal";
import KmSettlementModal from "@/components/KmSettlementModal";
import OdometerStartModal from "@/components/OdometerStartModal";
import { reconcileCustomerDocuments } from "@/lib/customerDocs";
import { suggestName, suggestAmount } from "@/lib/blockHints";
import { useIsMobile } from "@/lib/useIsMobile";
import { useModalChrome } from "@/lib/useModalChrome";
import { PL_MONTHS, PL_WD, fmtDate, toISODate, nowIceland } from "@/lib/dates";
import { matchesQuery } from "@/lib/search";
import type { Booking, BookingType, Customer, CustomerDocument } from "@/lib/types";
import { DOC_TYPES, customerLabel } from "@/lib/types";
import {
  ChevronLeft,
  ChevronRight,
  Plus,
...
export default function Timeline() {
  const { vehicles, bookings, removeBooking, updateBooking, updateCustomer, customerById } =
    useData();
  const showToast = useToast();
  const [sendingPay, setSendingPay] = useState(false);
  const [sendingPlaces, setSendingPlaces] = useState(false);
  ...
  // Natywna edycja danych klienta z drawera (zamiast kruchego iframe profilu): ten
  // sam CustomerFormModal co w kreatorze/profilu, wstępnie wypełniony + dokumenty.
  const [editCustomer, setEditCustomer] = useState<{
    customer: Customer;
    existing: CustomerDocument[];
    docs: CustomerDocValues;
  } | null>(null);
  // Rozliczenie kilometrów przy zwrocie + zamknięcie rezerwacji.
  const [kmSettle, setKmSettle] = useState<Booking | null>(null);
  const [odoStartFor, setOdoStartFor] = useState<Booking | null>(null);
  ...
  // Umowa powiązana z aktualnie otwartym wpisem (status podpisu w drawerze).
  ...
  useEffect(() => {
    ...
    if (!cid) {
      // defer, żeby nie wołać setState synchronicznie w ciele efektu
      Promise.resolve().then(() => alive && setSelectedContract(null));
      return () => {
        alive = false;
      };
    }
    fetchContracts(cid).then((list) => {
      if (!alive) return;
      const forBooking = list.filter((c) => c.bookingId === bid);
      // Najnowsza podpisana > najnowsza wysłana (lista posortowana malejąco po dacie).
      setSelectedContract(forBooking.find((c) => c.signedAt) ?? forBooking[0] ?? null);
    });
    return () => {
      alive = false;
    };
  ...
  const vehicleRows = useMemo(() => {
    const bookingHay = (b: Booking) => {
      ...
      return [cust?.name, cust?.companyName, b.notes, v?.name, v?.plate]
        .filter(Boolean)
        .join(" ");
    };
    // Pojazdy „inactive" = już nie w naszej flocie (sprzedane/oddane) — nie
    // pokazujemy ich wierszy w kalendarzu. Wpisy w bazie zostają (historia).
    return vehicles
  ...
  // Dzień (offset od `start`) najwcześniejszego wpisu pasującego do wyszukiwania —
  // żeby przy wpisaniu klienta przewinąć kalendarz DO niego (wcześniej pasek bywał
  // poza ekranem i trzeba było ręcznie skrolować w bok).
  const firstMatchOffset = useMemo(() => {
    if (!q) return null;
    let min = Infinity;
    for (const r of vehicleRows) {
      for (const b of r.rows) {
        const sC = parseISO(b.start) < start ? start : parseISO(b.start);
        const off = differenceInCalendarDays(sC, start);
        if (off >= 0 && off < min) min = off;
      }
    }
    return Number.isFinite(min) ? min : null;
  }, [q, vehicleRows, start]);

  // Przewiń poziomo do pierwszego trafienia (z małym marginesem z lewej). Ustawiamy
  // scrollLeft BEZPOŚREDNIO — `scrollTo({behavior:"smooth"})` bywa ignorowane (część
  // przeglądarek / kontener z overscroll-contain), a natychmiastowy skok i tak jest
  // pożądany: użytkownik chce od razu zobaczyć wyszukany wpis.
  useEffect(() => {
    if (!q || firstMatchOffset == null || !scrollRef.current) return;
    scrollRef.current.scrollLeft = Math.max(0, (firstMatchOffset - 1) * DAY_W);
  }, [q, firstMatchOffset, DAY_W]);
  ...
  // Pobierz dokumenty klienta i otwórz modal edycji (numer dowodu żyje w
  // customer_documents, więc dociągamy je przed montażem — modal czyta initialDocs raz).
  const openEditCustomer = async (customer: Customer) => {
    const existing = await fetchCustomerDocuments(customer.id);
    const idDoc = existing.find((d) => d.docType === DOC_TYPES[0]);
    const licDoc = existing.find((d) => d.docType === DOC_TYPES[1]);
    setEditCustomer({
      customer,
      existing,
      docs: {
        idNumber: idDoc?.docNumber,
        idIssued: idDoc?.issuedAt,
        idExpires: idDoc?.expiresAt,
        licIssued: licDoc?.issuedAt,
        licExpires: licDoc?.expiresAt,
      },
    });
  };

  // Etykieta paska na kalendarzu: nazwa klienta (firmowy → nazwa firmy) + początek
  // notatki, żeby rezerwacje dało się odróżnić i się nie myliły (np. „Lejdis trip").
  // Blok pokazuje samą notatkę (nie ma klienta). Zbyt długie i tak ucina CSS —
  // pełna treść jest w tooltipie (title).
  const barLabel = (b: Booking) => {
    const c = customerById(b.customerId);
    const who = c ? customerLabel(c) : null;
    const note = b.notes?.replace(/\s+/g, " ").trim();
    ...
    <div>
                      ...
                      return (
                        <button
                          ...
                          className={`z-10 flex items-center overflow-hidden text-ellipsis whitespace-nowrap rounded-md border px-2 text-xs font-medium ${s.bar} ${
                            ...
                            q ? "z-20 shadow-md ring-2 ring-zinc-900/40" : ""
                          }`}
                        >
                          <span className="truncate">{barLabel(b)}</span>
                        </button>
                      );
                    })}
                  </div>
      ...
      {selected && (
        <Drawer onClose={() => setSelected(null)} title="Szczegóły wpisu">
          ...
          <DetailRow
            label="Pojazd"
            value={(() => {
              const v = vehicles.find((veh) => veh.id === selected.vehicleId);
              if (!v) return "—";
          ...
          <DetailRow
            label="Klient"
            value={customerLabel(customerById(selected.customerId))}
          />
          <DetailRow
            label="Od"
          ...
          {selected.type === "reservation" && (
            <DetailRow
              label="Licznik wydania"
              value={
                selected.odometerStart != null
                  ? `${selected.odometerStart.toLocaleString("pl-PL")} km`
                  : "— (nie spisano)"
              }
            />
          )}
          {selected.odometerEnd != null && (
            <DetailRow
              label="Licznik zwrotu"
              value={`${selected.odometerEnd.toLocaleString("pl-PL")} km`}
            />
          )}
          <BookingNote
            key={selected.id}
          ...
          <div className="mt-6 space-y-2">
            ...
            {selected.customerId && (
              <button
                onClick={() => {
                  const c = customerById(selected.customerId);
            ...
            {selected.customerId && selectedContract && (
              <div
              ...
              >
                <span className="min-w-0 truncate">
                  {selectedContract.signedAt
                    ? `Umowa nr ${selectedContract.number} — podpisana ✓${selectedContract.signerName ? ` (${selectedContract.signerName})` : ""}`
                    : `Umowa nr ${selectedContract.number} — wysłana, czeka na podpis`}
                </span>
                {selectedContract.signToken && (
                  <button
                    onClick={() =>
                      setIframe({
                        title: `Umowa nr ${selectedContract.number}`,
                        url: `/sign/${selectedContract.signToken}`,
                      })
                    }
                    className="shrink-0 rounded-md border border-zinc-200 bg-white px-2 py-1 text-xs font-medium text-zinc-600 hover:bg-zinc-50"
                  >
                    Podgląd
                  </button>
                )}
              ...
              <button
                onClick={() =>
                  setIframe({
                    ...
                    url: `/invoices?customerId=${selected.customerId}&bookingId=${selected.id}`,
                  })
                }
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-3 text-sm font-medium text-zinc-700 hover:bg-zinc-50"
              >
                <ReceiptText className="size-4" /> Wyślij fakturę
              </button>
            )}
            {selected.customerId && (
              <button
            ...
            {selected.type === "reservation" && (
              <button
                onClick={() => setOdoStartFor(selected)}
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-3 text-sm font-medium text-zinc-700 hover:bg-zinc-50"
              >
                <Gauge className="size-4" />{" "}
                {selected.odometerStart != null
                  ? `Licznik wydania: ${selected.odometerStart.toLocaleString("pl-PL")} km`
                  : "Wpisz licznik przy wydaniu"}
              </button>
            )}
            {selected.type === "reservation" && (
              <button
                onClick={() => setKmSettle(selected)}
              ...
              >
                <Gauge className="size-4" /> Rozlicz km i zakończ
              </button>
            )}
            ...
            {customerById(selected.customerId)?.email && (
              <button
                onClick={async () => {
                  setSendingPlaces(true);
                  const res = await sendPlacesEmailAction({
                    bookingId: selected.id,
                    origin: window.location.origin,
                  });
                  setSendingPlaces(false);
                  showToast(
                    res.ok ? "success" : "error",
                    res.ok
                      ? `Mapa miejsc wysłana do ${customerById(selected.customerId)?.email}.`
                      : res.message ?? "Nie udało się wysłać maila.",
                  );
                }}
                disabled={sendingPlaces}
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-3 text-sm font-medium text-zinc-700 hover:bg-zinc-50 disabled:opacity-60"
              >
                {sendingPlaces ? <Loader2 className="size-4 animate-spin" /> : <MapPin className="size-4" />}
                Wyślij mapę miejsc (Islandia)
              </button>
            )}
            <button
      ...
      {editCustomer && (
        <CustomerFormModal
          ...
          initial={editCustomer.customer}
          initialDocs={editCustomer.docs}
          onClose={() => setEditCustomer(null)}
          onSubmit={async (patch, docs) => {
            const ok = await updateCustomer(editCustomer.customer.id, patch);
            if (!ok) return; // updateCustomer pokazał już toast + rollback
            const res = await reconcileCustomerDocuments(
              editCustomer.customer.id,
              patch,
              docs,
              editCustomer.existing,
            );
            showToast(
              res.ok ? "success" : "error",
              res.ok
                ? "Dane klienta zapisane."
                : "Dane zapisane, ale dokumentów nie — spróbuj ponownie w profilu klienta.",
            );
      ...
      {odoStartFor && (
        <OdometerStartModal
          key={odoStartFor.id}
          booking={odoStartFor}
          onClose={() => setOdoStartFor(null)}
          onDone={(odometerStart) => {
            // odśwież szufladę tą samą wartością, którą właśnie zapisano
            setSelected((s) =>
              s && s.id === odoStartFor.id ? { ...s, odometerStart } : s,
            );
          }}
        />
      )}
      ...
      {kmSettle && (
        <KmSettlementModal
          booking={kmSettle}
          customer={customerById(kmSettle.customerId)}
          onClose={() => setKmSettle(null)}
          onDone={() => {
            setKmSettle(null);
            setSelected(null);
          }}
