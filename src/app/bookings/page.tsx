...
import { Plus, X, Pencil, ChevronLeft, ChevronRight, Search, Mail, SearchX } from "lucide-react";
...
function BookingsContent() {
  ...
  const bookingHay = (b: Booking) => {
    ...
    return [
      ...
      customerById(b.customerId)?.companyName,
      b.notes,
      TYPE_LABEL[b.type],
  ...
  const { sorted: rows, sortKey, sortDir, toggleSort } = useSort(
    filtered,
    {
      ...
      customer: (b) => {
        ...
        return c ? customerLabel(c) : b.notes ?? "";
      },
            ...
            {rows.length === 0 ? (
              <tr>
                <td colSpan={9} className="px-4 py-2">
                  <EmptyState
                    icon={SearchX}
                    title="Brak rezerwacji spełniających kryteria"
                    hint={
                      query.trim()
                        ? `Nic nie pasuje do „${query.trim()}".`
                        : "Zmień miesiąc albo zdejmij filtr klienta."
                    }
                    action={
                      query.trim() ? (
                        <button
              ...
              rows.map((b) => {
                const days = differenceInCalendarDays(parseISO(b.end), parseISO(b.start));
                return (
                  <tr key={b.id} className="hover:bg-zinc-50">
                    ...
                    <td className="px-4 py-3 text-zinc-600">
                      {customerById(b.customerId) ? (
                        ...
                        >
                          {customerLabel(customerById(b.customerId))}
                        </button>
                      ) : (
                        (b.notes ?? "—")
                      )}
