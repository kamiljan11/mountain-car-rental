...
const inputCls =
  "w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-base outline-none transition-colors focus:border-zinc-900 md:text-sm focus:ring-2 focus:ring-zinc-900/10";
      ...
      {editing && (
        <CustomerFormModal
          ...
          onSubmit={async (patch, docs) => {
            ...
            if (!ok) return;
            // Dokumenty żyją w customer_documents — wspólny helper (ten sam co w
            // kreatorze i w kalendarzu). Wyczyszczenie numeru dowodu NIE usuwa
            // dokumentu (od tego jest zakładka Dokumenty).
            const { changed } = await reconcileCustomerDocuments(id, patch, docs, documents);
