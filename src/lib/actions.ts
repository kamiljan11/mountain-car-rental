...
export async function sendContractForSignatureAction(input: {
  contractId: string;
  origin: string;
  toOverride?: string;
}) {
  return db.sendContractForSignature(input);
}

/* ---------- Faktury ---------- */

export async function fetchInvoicesAction(customerId?: string) {
  return db.fetchInvoices(customerId);
}

export async function insertInvoiceAction(input: {
  number: string;
  customerId?: string;
  vehicleId?: string;
  bookingId?: string;
  companyKey?: string;
  paymentMethod?: import("./invoice").PaymentMethod;
  paymentTerm?: import("./invoice").PaymentTerm;
...
export async function sendInvoiceEmailAction(input: {
  ...
  return db.sendInvoiceEmail(input);
}
...
export async function sendPlacesEmailAction(input: { bookingId: string; origin: string }) {
  return db.sendPlacesEmail(input);
}
